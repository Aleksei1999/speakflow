...
/** Мягкая версия для read-only fetch: возвращает null вместо throw,
 *  чтобы preview-режим без auth не крашил UI. */
async function tryGetTeacher() {
  try {
...
export async function fetchChatMessages(studentId: string): Promise<ChatMessage[]> {
  ...
  if (!auth) return [] // preview / not logged in — тихо, без throw
