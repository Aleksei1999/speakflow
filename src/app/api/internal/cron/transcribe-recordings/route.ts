...
//
// Concurrency-safe: захват записи через атомарный UPDATE `locked_at`
// (см. миграция 20260904200000). Если lock не удался — другой тик уже
...
// Recovery-window: если процесс упал/зависла — через это время
// следующий тик считает lock истёкшим и захватит запись повторно.
...
export async function POST(req: NextRequest) {
  ...
  // Берём с запасом (20), чтобы если верхние захвачены другим worker'ом —
  // можно было попробовать следующего кандидата.
  const { data: candidates, error: candErr } = await admin
    .from("lesson_recordings")
    .select("id, lesson_id, storage_prefix, duration_sec, finalized_at, locked_at")
    .eq("status", "finalized")
  ...
  const eligible = candidates.filter((c) => !okSet.has(c.id) && (failedCounts.get(c.id) ?? 0) < MAX_ATTEMPTS)
  if (eligible.length === 0) {
    return NextResponse.json({ ok: true, picked: null })
  }

  // Захватываем lock атомарно. UPDATE ... WHERE (не заблокирован
  // или lock истёк) RETURNING id — гарантирует ровно одного winner'а.
  // Проходим по кандидатам по порядку: если первый уже захвачен, пробуем
  // следующего — не хотим ждать полный тик из-за одного busy-worker.
  ...
  for (const cand of eligible) {
    const { data: locked, error: lockErr } = await admin
      ...
      .select("id")
    if (lockErr) {
      console.error("[cron/transcribe] lock attempt failed:", lockErr)
      continue
    }
  ...
  if (!target) {
    console.log("[cron/transcribe] all candidates locked by other workers")
  ...
  // release-lock снимаем в finally, чтобы даже throw не оставил запись
  // заблокированной до истечения TTL.
  const releaseLock = async () => {
    const { error } = await admin
      .from("lesson_recordings")
      .update({ locked_at: null })
      .eq("id", target!.id)
    if (error) console.error("[cron/transcribe] release lock failed:", error)
  }
